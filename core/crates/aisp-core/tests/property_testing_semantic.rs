...
fn document_with_types() -> impl Strategy<Value = String> {
    prop::collection::vec(type_name(), 1..=5).prop_map(|type_names| {
        let type_defs = type_names
            .iter()
            .map(|name| format!("  {}≜ℕ", name))
            .collect::<Vec<_>>()
            .join("\n");

        let rules = type_names
            .iter()
...
fn document_with_conflicts() -> impl Strategy<Value = String> {
    (type_name(), variable_name()).prop_map(|(type_name, var_name)| {
        format!(
            r#"𝔸5.1.conflicts@2026-01-26
...
⟦Ε⟧⟨δ≜0.5⟩"#,
            type_name, type_name, var_name, type_name, var_name, type_name, var_name, var_name
        )
    })
}
...
fn document_with_symbol_density() -> impl Strategy<Value = String> {
    (1..=10usize, 0.1f64..=0.9).prop_map(|(complexity, target_delta)| {
...
⟦Ε⟧⟨δ≜{:.3}⟩"#,
            meta_entries, type_entries, target_delta
        )
    })
}
