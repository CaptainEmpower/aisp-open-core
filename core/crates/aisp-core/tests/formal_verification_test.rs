...
use aisp_core::{
    ast::*, error::AispResult, formal_verification::*, property_types::*,
    semantic::SemanticAnalyzer, validator::AispValidator,
};
...
fn test_formal_verification_basic() {
    ...
    assert!(
        result.is_ok(),
        "Simple conjunction should be verifiable: {:?}",
        result.err()
    );
    ...
    assert!(
        !proof.proof_steps.is_empty(),
        "Proof should have actual steps"
    );
    assert_eq!(
        proof.validation,
...
fn test_ambiguity_measurement() {
    ...
    assert!(
        result.ambiguity > 0.1,
        "Ambiguous document should have high ambiguity: {}",
        result.ambiguity
    );
    ...
    assert!(
        ...
        "Formal document should have low ambiguity: {}",
        result.ambiguity
    );
    assert!(
        result.ambiguity < 0.02,
        "Should satisfy AISP ambiguity invariant: {}",
        result.ambiguity
    );
}
...
fn test_enumeration_parsing() {
    ...
    assert!(
        result.is_ok(),
        "Space-separated enumerations should parse: {:?}",
        result.err()
    );
    ...
    assert!(
        result2.is_ok(),
        "Comma-separated enumerations should parse: {:?}",
        result2.err()
    );
}
...
fn test_theorem_prover_logic() {
    ...
    assert!(
        result.is_ok(),
        "Simple tautology P → P should be provable: {:?}",
        result.err()
    );
}
...
fn test_quality_assessment() {
    ...
    assert!(
        result.delta > 0.8,
        "Well-formed document should have high delta: {}",
        result.delta
    );
    assert!(
        result.tier.value() >= 3,
        "Should achieve high quality tier: {:?}",
        result.tier
    );
...
fn test_property_extraction_pipeline() {
    ...
    assert!(
        result.valid || !result.error.unwrap_or_default().is_empty(),
    ...
    assert!(
        result.delta > 0.5,
        "Document with formal properties should have decent delta"
    );
}
...
fn test_end_to_end_formal_verification() {
    ...
    assert!(
        result.is_ok(),
        "Complete formal document should validate: {:?}",
        result.err()
    );

    let validation = result.unwrap();
    assert!(
        validation.valid || validation.warnings.len() < 5,
        "Should be valid or have minimal warnings"
    );
    assert!(
        validation.ambiguity < 0.02,
        "Should satisfy AISP ambiguity invariant"
    );
    assert!(
        validation.delta > 0.9,
        "Should achieve high semantic density"
    );
}
...
fn test_verification_performance() {
    ...
    assert!(
        ...
        "Verification should complete quickly: {}ms",
        duration.as_millis()
    );
}
...
fn test_formal_verification_demonstration() {
    ...
    assert!(
        result.ambiguity < 0.02,
        "Demo should satisfy ambiguity invariant"
    );
    assert!(
        result.tier.value() >= 3,
        "Demo should achieve high quality tier"
    );
}
