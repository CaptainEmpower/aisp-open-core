...
mod tests {
    use super::*;
    use crate::{
        invariant_types::{EvidenceType, InvariantEvidence, InvariantType},
        property_types::{AtomicFormula, FormulaStructure, PropertyFormula, SourceLocation, Term},
    };
    ...
    fn test_multiple_invariants_grouping() {
        ...
        assert!(!output.is_empty());
        assert!(output.contains("2") || output.contains("multiple") || output.len() > 100);
