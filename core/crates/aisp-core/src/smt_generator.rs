...
impl SMTGenerator {
    ...
    pub fn generate_smt_program(
        &mut self,
        properties: &[ExtractedProperty],
    ) -> AispResult<SMTGenerationResult> {
    ...
    fn generate_basic_sorts(&mut self, script: &mut String) -> AispResult<()> {
        ...
        for (aisp_sort, smt_sort) in &basic_sorts {
            if matches!(*smt_sort, "Int" | "Bool" | "Real" | "String") {
                ...
                self.type_env
                    .insert(aisp_sort.to_string(), smt_sort.to_string());
            } else {
                writeln!(script, "(declare-sort {} 0)", aisp_sort)?;
                self.sort_declarations.insert(aisp_sort.to_string());
                self.type_env
                    .insert(aisp_sort.to_string(), aisp_sort.to_string());
            }
        }
    ...
    fn extract_sorts_from_properties(
        &mut self,
        properties: &[ExtractedProperty],
        script: &mut String,
    ) -> AispResult<()> {
    ...
    fn extract_sorts_from_formula(
        &self,
        formula: &PropertyFormula,
        custom_sorts: &mut HashSet<String>,
    ) -> AispResult<()> {
    ...
    fn extract_functions_from_properties(
        &mut self,
        properties: &[ExtractedProperty],
        script: &mut String,
    ) -> AispResult<()> {
    ...
    fn generate_property_assertion(
        &mut self,
        property: &ExtractedProperty,
        assertion_name: &str,
        script: &mut String,
    ) -> AispResult<SMTExpectedResult> {
...
mod tests {
    ...
    fn test_property_assertion_generation() -> AispResult<()> {
        ...
        let mut script = String::new();
        let result =
            generator.generate_property_assertion(&property, "test_assertion", &mut script)?;
    ...
    fn test_reset_functionality() {
        ...
        generator
            .type_env
            .insert("Test".to_string(), "TestSort".to_string());
        generator
            .function_env
            .insert("f".to_string(), "test_func".to_string());
