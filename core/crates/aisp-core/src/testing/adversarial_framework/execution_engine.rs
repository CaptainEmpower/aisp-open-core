...
use super::types::*;
use crate::parser::robust_parser::{ParseResult, RobustAispParser, SecuritySeverity};
use crate::parser::unicode_support::{SecurityReport, UnicodeSymbolRegistry};
...
impl AttackExecutionEngine {
    ...
    fn run_parse_bypass_tests(&self, test_suite: &AdversarialTestSuite) -> Vec<AttackResult> {
        test_suite
            ...
            .map(|attack| {
                ...
                let success =
                    self.evaluate_parse_bypass_success(&parse_result, &attack.expected_behavior);
                let bypass_achieved =
                    parse_result.document.is_some() && parse_result.errors.is_empty();
        ...
        test_suite
            ...
            .map(|attack| {
                ...
                let security_report = self
                    .unicode_registry
                    .generate_security_report(&attack.malicious_payload);
                let parsing_time = start_time.elapsed();

                let success = self.evaluate_unicode_attack_success(&security_report, &parse_result);
                let bypass_achieved =
                    parse_result.document.is_some() && security_report.dangerous_characters == 0;
    ...
    fn run_boundary_condition_tests(&self, test_suite: &AdversarialTestSuite) -> Vec<AttackResult> {
        test_suite
            ...
            .map(|attack| {
                ...
                let success = self
                    .evaluate_boundary_attack_success(&parse_result, &attack.expected_failure_mode);
                let bypass_achieved =
                    parse_result.document.is_some() && parse_result.errors.is_empty();
    ...
    fn run_malformed_document_tests(&self, test_suite: &AdversarialTestSuite) -> Vec<AttackResult> {
        test_suite
            ...
            .map(|attack| {
                ...
                let success = self.evaluate_malformed_attack_success(
                    &parse_result,
                    &attack.expected_recovery_behavior,
                );
                let bypass_achieved =
                    parse_result.document.is_some() && !parse_result.recovery_applied;
                let detection_triggered =
                    parse_result.recovery_applied || !parse_result.security_issues.is_empty();
    ...
    fn run_resource_exhaustion_tests(
        &self,
        test_suite: &AdversarialTestSuite,
    ) -> Vec<AttackResult> {
        test_suite
            ...
            .map(|attack| {
                ...
                let success = parsing_time
                    > std::time::Duration::from_millis(attack.threshold_limits.cpu_time_ms as u64);
                let bypass_achieved =
                    parse_result.document.is_some() && parse_result.security_issues.is_empty();
    ...
    fn run_injection_tests(&self, test_suite: &AdversarialTestSuite) -> Vec<AttackResult> {
        test_suite
            ...
            .map(|attack| {
                let payload = format!(
                    "𝔸5.1.test@2026-01-27⟦Ω:Meta⟧{{{}}}",
                    attack.injection_payload
                );
                let start_time = Instant::now();
                let parse_result = self.parser.parse(&payload);
                let parsing_time = start_time.elapsed();
    ...
    fn generate_resource_exhaustion_payload(&self, attack: &ResourceExhaustionAttack) -> String {
        match attack.payload_generator {
            ...
            PayloadGenerator::RepeatedPattern => {
                format!(
                    "𝔸5.1.test@2026-01-27{}",
                    "⟦Ω:Meta⟧{Vision≜\"test\"}".repeat(attack.threshold_limits.max_iterations)
                )
            }
            PayloadGenerator::LargeInput => {
                format!(
                    "𝔸5.1.test@2026-01-27⟦Ω:Meta⟧{{Vision≜\"{}\"}}",
                    "A".repeat(attack.threshold_limits.memory_mb * 1024)
                )
            }
    ...
    fn generate_assessment_report(
        ...
        let critical_vulnerabilities = attack_results
            .iter()
    ...
    fn evaluate_parse_bypass_success(
        &self,
        result: &ParseResult,
        ...
        match expected {
            ...
            ExpectedBehavior::ShouldParseWithWarnings => {
                result.document.is_some() && !result.warnings.is_empty()
            }
            ExpectedBehavior::ShouldTriggerSecurity => !result.security_issues.is_empty(),
            ExpectedBehavior::ShouldBypassValidation => {
                result.document.is_some() && result.errors.is_empty()
            }
        }
    }

    fn evaluate_unicode_attack_success(
        &self,
    ...
    fn evaluate_boundary_attack_success(
        &self,
        result: &ParseResult,
        ...
        match expected_failure {
            ...
            FailureMode::IncorrectBehavior => {
                result.document.is_some() && !result.errors.is_empty()
            }
        }
    }

    fn evaluate_malformed_attack_success(
        &self,
        result: &ParseResult,
        expected_recovery: &RecoveryBehavior,
    ) -> bool {
        match expected_recovery {
            RecoveryBehavior::GracefulDegradation => {
                result.document.is_some() && result.recovery_applied
            }
    ...
    pub fn generate_vulnerability_summary(&self, results: &[AttackResult]) -> VulnerabilitySummary {
        let critical_issues = results
            ...
            .filter(|r| r.security_impact == SecuritySeverity::Critical)
            .count();
        let high_risk_issues = results
            .iter()
            .filter(|r| r.security_impact == SecuritySeverity::High)
            .count();
        let medium_risk_issues = results
            .iter()
            .filter(|r| r.security_impact == SecuritySeverity::Medium)
            .count();
        let low_risk_issues = results
            .iter()
            .filter(|r| r.security_impact == SecuritySeverity::Low)
            .count();

        let most_critical_attack = results
            .iter()
        ...
        if results
            .iter()
            .any(|r| r.attack_category == AttackCategory::ParseBypass && r.success)
        {
...
mod tests {
    ...
    fn test_security_score_calculation() {
        ...
        let bad_results = vec![AttackResult {
