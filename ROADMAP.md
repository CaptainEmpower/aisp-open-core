...
| R-13 | Deduplicate the five overlapping `formal_verification_*` test files into one suite | Single formal-verification test entry point; no redundant suites | Done — ten suites gated behind nonexistent `*-deprecated` features removed; `property_testing_formal` restored to current APIs. Consolidated the overlapping integration suites (`formal_integration_simple`, `simple_integration`, `working_integration`) into two SRP-aligned entry points — `formal_verification_integration` (formal/Z3 path) and `validation_pipeline_integration` (parser→validator pipeline) — deduplicating identical scenarios with zero coverage loss; `satisfiability_integration` and `integration_comprehensive` kept as distinct suites |
